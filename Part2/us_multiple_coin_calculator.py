...
# entered then exclude_denomination returns 'incorrect_usage',
...
# Calculates the amount of each coin the user can recieve for their cents, prioritising the higher denominations.
# The results are returned in a list, us_coins_amounts.
# Each entry in the list corresponds to the coin in the same entry of the list us_coins.
# E.g us_coins_amounts[1] corresponds to us_coins[1], which is $1.
# us_coins_amounts is one element longer than us_coins, however.
# The extra (final) element corresponds to the amount of cents that are left over.
# The input excluded_denomination takes the denomination that needs to be excluded.
# E.g calculate(1234, $2) would exclude $2 and us_coins_amounts would read [0, 12, 0, 1, 1, 4].
# 12 $1 coins, 1 20c, 1 10c and 4 cents left over.
